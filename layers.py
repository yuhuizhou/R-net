...
def pointer_net(passage, passage_len, question, question_len, cell, params, scope = "pointer_network"):
    with tf.variable_scope(scope):
        ...
        passage = mask_memory(passage, passage_len)
        question = mask_memory(question, question_len)
        ...
        p1_logits = attention(inputs, Params.attn_size, weights_p, output_fn = None, memory_len = passage_len, scope = "attention")
        ...
        p2_logits = attention(inputs, Params.attn_size, weights_p, output_fn = None, memory_len = passage_len, scope = "attention", reuse = True)
...
def gated_attention(memory, inputs, states, units, params, self_matching = False, output_argmax = None, memory_len = None, scope="gated_attention"):
    with tf.variable_scope(scope):
        weights, W_g = params
        # memory masking
        if memory_len is not None:
            sequence_mask = tf.expand_dims(tf.sequence_mask(memory_len, maxlen = memory.shape[1], dtype = tf.float32),-1)
            memory *= sequence_mask
...
def mask_memory(memory, memory_sequence_length):
    memory_mask = tf.expand_dims(tf.sequence_mask(memory_sequence_length, maxlen = memory.shape[1], dtype = tf.float32),-1)
    return memory * memory_mask

def mask_attn_score(score, memory_sequence_length, score_mask_value = float("-inf")):
    score_mask = tf.sequence_mask(
        memory_sequence_length, maxlen=score.shape[1])
    score_mask_values = score_mask_value * tf.ones_like(score)
    return tf.where(score_mask, score, score_mask_values)

def attention(inputs, units, weights, scope = "attention", output_fn = "softmax", memory_len = None, reuse = None):
    with tf.variable_scope(scope, reuse = reuse):
        outputs_ = []
