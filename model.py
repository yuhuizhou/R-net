...
class Model(object):
    def __init__(self,is_training = True, vocab_size = 100000):
        ...
        with self.graph.as_default():
            ...
            self.passage_w_len = tf.reduce_sum(tf.cast(self.passage_mask, tf.int32), axis=1)
            self.question_w_len = tf.reduce_sum(tf.cast(self.question_mask, tf.int32), axis=1)

            self.passage_cmask = tf.cast(1 - tf.cast(tf.equal(self.passage_c,1), tf.float32), tf.bool)
            self.question_cmask = tf.cast(1 - tf.cast(tf.equal(self.question_c,1), tf.float32), tf.bool)
            self.passage_c_len = tf.reduce_sum(tf.cast(self.passage_cmask, tf.int32), axis=2)
            self.question_c_len = tf.reduce_sum(tf.cast(self.question_cmask, tf.int32), axis=2)
        ...
        with tf.variable_scope("attention_match_rnn"):
            ...
            for i in range(2):
                args = {"inputs": inputs,
                        ...
                        "memory": memory,
                        "memory_mask": self.question_mask if i == 0 else self.passage_mask,
    ...
    def pointer_network(self):
        ...
        self.points_logits = pointer_net(self.final_bidirectional_outputs, self.passage_mask, self.question_encoding, self.question_mask, cell, is_training = self.is_training, dropout = self.dropout, scope = "pointer_network")
...
def debug():
    ...
    model = Model(is_training = True, vocab_size = dict_.vocab_size)
    print("Built model")

def test():
    dict_ = pickle.load(open(Params.data_dir + "dictionary.pkl","r"))
    model = Model(is_training = False, vocab_size = dict_.vocab_size); print("Built model")
...
def main():
    dict_ = pickle.load(open(Params.data_dir + "dictionary.pkl","r"))
    model = Model(is_training = True, vocab_size = dict_.vocab_size); print("Built model")
    init = False
    ...
    if not os.path.isfile(os.path.join(Params.logdir,"checkpoint")):
        ...
        glove = np.reshape(glove,(model.vocab_size,Params.emb_size))
    with model.graph.as_default():
